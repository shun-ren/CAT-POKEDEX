"""Evaluate the trained CATDEX coat ResNet18 classifier on coats/test only."""
...
def save_confusion_matrix(matrix, classes):
    figure, axis = plt.subplots(figsize=(max(10, len(classes) * 0.7), max(8, len(classes) * 0.7)))
    image = axis.imshow(matrix, cmap="Blues")
    figure.colorbar(image, ax=axis)
    axis.set_title("Coat test confusion matrix")
    axis.set_xlabel("Predicted label")
    axis.set_ylabel("True label")
    axis.set_xticks(range(len(classes)))
    axis.set_yticks(range(len(classes)))
    axis.set_xticklabels(classes, rotation=90, fontsize=8)
...
def main():
    ...
    if test_set.classes != classes:
        raise SystemExit("Test coat class folders do not match the classes stored in the checkpoint.")
    ...
    print(f"\nCOAT EVALUATION | device={device} | checkpoint epoch={checkpoint.get('epoch')} | test images={len(test_set)}\n")
    truth = []
    predicted = []
    with torch.no_grad():
        for images, targets in tqdm(test_loader, desc="Evaluating coat", unit="batch", dynamic_ncols=True):
